...
class BlancoBot(Bot):
  ...
  async def on_application_command_error(self, itx: Interaction, error: Exception):
    ...
    try:
      ...
      self._logger.warning(
        'Error 404 while sending error msg for interaction %d', itx.id
      )
  ...
  async def on_node_ready(self, node: 'Node'):
    ...
    if node.session_id is not None:
      ...
      if old_id is not None and old_id != node.session_id:
        self._logger.debug(
          "Replacing old session ID `%s' for node `%s'", old_id, node.label
        )
  ...
  async def on_track_start(self, event: 'TrackStartEvent[Jockey]'):
    ...
    self._logger.info("Started playing `%s' in %s", event.track.title, guild.name)

    # Send now playing embed
    try:
      await self.send_now_playing(event)
    except EndOfQueueError:
      self._logger.warning('Got track_start event for idle player in %s', guild.name)
      return
    ...
    if self._config.match_ahead:
      ...
      self._logger.debug("Matching next track `%s' in the background", next_track.title)
  ...
  async def on_track_end(self, event: 'TrackEndEvent[Jockey]'):
    ...
    if event.reason == EndReason.REPLACED:
      self._logger.warning(
        "Skipped `%s' in %s", event.track.title, event.player.guild.name
      )
      ...
      self._logger.info('Stopped player in %s', event.player.guild.name)
    elif event.reason == EndReason.LOAD_FAILED:
      self._logger.critical(
        "Failed to load `%s' in %s", event.track.title, event.player.guild.name
      )
  ...
  def get_spotify_client(self, user_id: int) -> Optional[PrivateSpotify]:
    ...
    if creds is None:
      ...
      raise ValueError(
        f'Please link your Spotify account [here.]({self._config.base_url})'
      )

    # Check if a client already exists
    if user_id not in self._spotify_clients:
      self._spotify_clients[user_id] = PrivateSpotify(
        config=self._config, database=self._db, credentials=creds
      )
  ...
  async def init_pool(self):
    ...
    for node in nodes.values():
      ...
      try:
        ...
        self._logger.debug("Using session ID `%s' for node `%s'", session_id, node.id)
      ...
      try:
        ...
        self._logger.error("Lavalink node `%s' refused connection", node.id)

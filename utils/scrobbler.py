...
class Scrobbler:
  ...
  def __init__(self, config: 'Config', creds: 'LastfmAuth', logger: 'Logger'):
    ...
    self._net = pylast.LastFMNetwork(
      api_key=config.lastfm_api_key, api_secret=config.lastfm_shared_secret
    )
  ...
  def scrobble(self, track: 'QueueItem'):
    ...
    try:
      ...
      self._logger.error(
        "Error scrobbling `%s' for user %d: %s", track.title, self._user_id, err
      )
      raise

    self._logger.debug("Scrobbled `%s' for user %d", track.title, self._user_id)

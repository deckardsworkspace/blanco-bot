...
class Database:
  ...
  def get_last_bump(self, guild_id: int) -> int:
    ...
    """
    self._cur.execute(
      f'SELECT last_bump FROM player_settings WHERE guild_id = {guild_id}'
    )
    return self._cur.fetchone()[0]
  ...
  def get_bumps(self, guild_id: int) -> Optional[List[Bump]]:
    ...
    return [
      Bump(idx=row[0], guild_id=row[1], url=row[2], title=row[3], author=row[4])
      for row in rows
    ]
  ...
  def get_bump(self, guild_id: int, idx: int) -> Optional[Bump]:
    ...
    if row is None:
      return None
    return Bump(idx=row[0], guild_id=row[1], url=row[2], title=row[3], author=row[4])

...
from bot.utils.player_checks import check_mutual_voice
from bot.views.spotify_dropdown import SpotifyDropdownView
...
class PlayerCog(Cog):
  ...
  async def on_voice_state_update(
    ...
    if jockey is not None:
      ...
      if (
        hasattr(jockey.channel, 'members')
        and len(jockey.channel.members) == 1  # type: ignore
      ...
      if member.id == member.guild.me.id and before.deaf and not after.deaf:
        await self._deafen(
          member.guild.me, was_deafened=True, channel=jockey.status_channel
        )
  ...
  async def _deafen(
    ...
    if channel is not None and hasattr(channel, 'send'):
      ...
      try:
        ...
        self._logger.error(
          'Unable to send deafen message in guild %d', bot_user.guild.id
        )
  ...
  async def _disconnect(
    ...
    try:
      ...
      self._logger.error(
        'Unable to send disconnect message in guild %d', jockey.guild.id
      )
  ...
  async def jump(
    ...
    if position < 1 or position > len(jockey.queue):
      await itx.response.send_message(
        f'Specify a number from 1 to {str(len(jockey.queue))}.', ephemeral=True
      )
      return
    ...
    try:
      ...
      await itx.followup.send(
        embed=create_success_embed(f'Jumped to track {str(position)}')
      )
  ...
  async def loop(self, itx: Interaction):
    ...
    return await itx.response.send_message(
      embed=create_success_embed('Looping current track')
    )
  ...
  async def loopall(self, itx: Interaction):
    ...
    return await itx.response.send_message(
      embed=create_success_embed('Looping entire queue')
    )
  ...
  async def play(
    ...
    """
    if (
      not isinstance(itx.user, Member)
      or not itx.user.voice
    ...
    if (
      self._bot.config.base_url is not None
      and self._bot.config.lastfm_api_key is not None
      and self._bot.config.lastfm_shared_secret is not None
    ):
  ...
  async def playlist(self, itx: Interaction):
    ...
    try:
      ...
      return await itx.followup.send(
        embed=create_error_embed(err.args[0]), ephemeral=True
      )
    ...
    if len(playlists) == 0:
      return await itx.followup.send(
        embed=create_error_embed(message='You have no playlists.'),
        ephemeral=True,
      )
  ...
  async def remove(
    self,
    itx: Interaction,
    position: int = SlashOption(description='Position to remove', required=True),
  ):
    """
    ...
    await itx.followup.send(
      embed=create_success_embed(
        title='Removed from queue', body=f'**{title}**\n{artist}'
      )
    )
  ...
  async def search(
    ...
    try:
      ...
      return await itx.followup.send(
        embed=create_error_embed(message=f'No results found for `{query}`.'),
        ephemeral=True,
      )
  ...
  async def volume(
    ...
    if not volume:
      ...
      return await itx.response.send_message(
        f'The volume is set to {jockey.volume}.', ephemeral=True
      )

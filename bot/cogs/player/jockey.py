...
class Jockey(Player['BlancoBot']):
  ...
  def __init__(self, client: 'BlancoBot', channel: 'Connectable'):
    ...
    self._logger.info("Using node `%s' for %s", self.node.label, channel.guild.name)
    ...
    try:
      ...
      if not auto:
        await self.status_channel.send(
          embed=create_error_embed('Attempted to skip while disconnected')
        )
    ...
    if item.lavalink_track is None:
      try:
        ...
        item.lavalink_track = await find_lavalink_track(
          self.node, item, deezer_enabled=deezer_enabled
        )
  ...
  async def disconnect(self, *, force: bool = False):
    ...
    if np_msg is not None:
      try:
        ...
        self._logger.warning(
          'Failed to remove now playing message for %s', self.guild.name
        )
  ...
  async def play_impl(self, query: str, requester: int) -> str:
    ...
    try:
      new_tracks = await parse_query(self.node, self._bot.spotify, query, requester)
  ...
  async def resume(self):
    ...
    self._pause_ts = None
    self._logger.debug(
      'Unpaused beyond %d sec threshold, re-enqueueing', UNPAUSE_THRESHOLD
    )
  ...
  async def skip(self, *, forward: bool = True, index: int = -1, auto: bool = True):
    ...
    if index != -1:
      try:
        ...
        await self.status_channel.send(
          embed=create_error_embed(f'Unable to skip to index {index}')
        )
        raise
    ...
    while True:
      ...
      next_artist = (
        next_track.artist if next_track.artist is not None else 'Unknown artist'
      )

...
def log_failure(retry_state: RetryCallState) -> None:
  ...
  RETRY_LOGGER.warning(
    'Retrying %s(), attempt %s', func_name, retry_state.attempt_number
  )
...
class Spotify:
  ...
  def get_tracks(
    self, list_type: str, list_id: str
  ) -> Tuple[str, str, List[SpotifyTrack]]:
    """
    ...
    while True:
      if list_type == 'album':
        ...
        response = self._client.playlist_items(
          list_id, offset=offset, fields=fields, additional_types=['track']
        )
    ...
    if list_type == 'playlist':
      return (
        list_name,
        list_author,
        [extract_track_info(x) for x in tracks if x['track'] is not None],
      )
    return (
      list_name,
      list_author,
      [extract_track_info(x, list_artwork, album_name=list_name) for x in tracks],
    )

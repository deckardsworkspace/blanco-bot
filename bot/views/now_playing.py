...
from bot.utils.constants import SPOTIFY_403_ERR_MSG
...
class ShuffleButton(Button):
  ...
  def __init__(self, init_state: bool = False):
    ...
    """
    super().__init__(
      style=ButtonStyle.grey, label='Unshuffle' if init_state else 'Shuffle'
    )
...
class NowPlayingView(View):
  ...
  def __init__(
    self, bot: 'BlancoBot', player: 'Jockey', spotify_id: Optional[str] = None
  ):
  ...
  async def like(self, _: 'Button', interaction: 'Interaction'):
    ...
    try:
      ...
      return await interaction.followup.send(
        embed=create_error_embed(message), ephemeral=True
      )
    ...
    # Send response
    return await interaction.followup.send(
      embed=create_success_embed('Added to your Liked Songs.'), ephemeral=True
    )
